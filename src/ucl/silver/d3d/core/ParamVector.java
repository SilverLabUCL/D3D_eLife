 ...
 * classes (Diffusant, Detector, etc) thereby inhereting the functions defined below.
 *
 ...
 *
 * Finally, use the "set" functions to set parameter values
 * (e.g. DiffusantSimple.set("d", 0.33))
 */
...
public class ParamVector implements Serializable {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            try {
                ...
                if (type.equalsIgnoreCase("int")) {
                    addParam(this, NAME, fields[i].getInt(this), units, help);
                    count++;
                } else if (type.equalsIgnoreCase("double")) {
                    addParam(this, NAME, fields[i].getDouble(this), units, help);
                    count++;
                } else if (type.equalsIgnoreCase("boolean")) {
                    addParam(this, NAME, fields[i].getBoolean(this), help);
                    count++;
                } else if (type.equals("class java.lang.String")) {
                    addParam(this, NAME, fields[i].get(this).toString(), units, help);
                    count++;
                } else {
    ...
    private void updateParams(ParamObject[] v, ParamVector pv, Class c) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            try {
                ...
                if (type.equalsIgnoreCase("int")) {
                    o.setValue(fields[i].getInt(pv));
                } else if (type.equalsIgnoreCase("double")) {
                    o.setValue(fields[i].getDouble(pv));
                } else if (type.equalsIgnoreCase("boolean")) {
                    if (fields[i].getBoolean(pv)) {
                        o.setValue(1);
                    } else {
                        o.setValue(0);
                    }
                } else if (type.equals("class java.lang.String")) {
                    o.setValue(fields[i].get(pv).toString());
                } else {
                    // skip everything else
                    //Master.log(f);
                }
    ...
    public ParamObject getParamObject(ParamObject[] v, ParamVector pv, String name) {
        ...
        for (int i = 0; i < vector.length; i++) {
            ...
            if ((o.paramVector == pv) && name.equalsIgnoreCase(o.getName())) {
                return o;
            }
    ...
    public boolean set(ParamObject o, String str) {
        ...
        if (o.getType().equalsIgnoreCase("int") || o.getType().equalsIgnoreCase("double")) {
            ...
            d = Double.valueOf(str.toString());

            if (setMyParams(o, d.doubleValue())) {
                return o.setValue(d.doubleValue());
            }
    ...
    public void updateUsers() {
        ...
        for (int i = 0; i < userRegistry.length; i++) {
            if (userRegistry[i].getVector() != null) {
                updateVector(userRegistry[i].getVector());
