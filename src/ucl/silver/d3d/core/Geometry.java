...
public class Geometry extends CoordinatesVoxels {
    ...
    public void init() {
        ...
        if (PSFs != null) {
            ...
            for (int i = 0; i < PSFs.length; i++) {
                PSFs[i].init();
                PSFs[i].useGeometryCoordinates = true; // force this option
            }
    ...
    public final boolean isSpace(int xVoxel, int yVoxel, int zVoxel) {
        if ((checkBounds(xVoxel, yVoxel, zVoxel)) && (space[xVoxel][yVoxel][zVoxel] >= 0)) {
            return true;
        } else {
            return false;
        }
    ...
    public final boolean checkBounds(int xVoxel, int yVoxel, int zVoxel) {
        ...
        if ((xVoxel >= 0) && (yVoxel >= 0) && (zVoxel >= 0) && (xVoxel < space.length) && (yVoxel < space[0].length) &&
                (zVoxel < space[0][0].length)) {
            return true;
        }
        return false;
    }
    ...
    public final void setSpace(CoordinatesVoxels c, int what) {
    ...
    public final void ellipsoid(int Xscale, int Yscale, int Zscale) { // create elliptical space
        ...
        yScale = Yscale;
        zScale = Zscale;
        GeometryTools.ellipsoid(space, this, 1);
        checkSpace();
    }

    public final void ellipsoid(CoordinatesVoxels c) { // create elliptical space
        fill();
        GeometryTools.ellipsoid(space, c, 1);
        checkSpace();
    }

    public final void resizeAndFill(int Xvoxels, int Yvoxels, int Zvoxels, int fillValue) {
        resize(Xvoxels, Yvoxels, Zvoxels);
        setSpace(fillValue);
    }

    public final boolean resize(double Xwidth, double Ywidth, double Zwidth) {
    ...
    public final boolean resize(int Xvoxels, int Yvoxels, int Zvoxels) {
        ...
        if (forceEvenVoxels) {
            Xvoxels = D3DUtility.makeEven(Xvoxels);
            Yvoxels = D3DUtility.makeEven(Yvoxels);
            Zvoxels = D3DUtility.makeEven(Zvoxels);
        } else if (forceOddVoxels) {
            Xvoxels = D3DUtility.makeOdd(Xvoxels);
            Yvoxels = D3DUtility.makeOdd(Yvoxels);
            Zvoxels = D3DUtility.makeOdd(Zvoxels);
        }
    ...
    public void resizePSFs() {
        ...
        for (int i = 0; i < PSFs.length; i++) {
            ...
            PSFs[i].useGeometryCoordinates = true;

            if (PSFs[i].array != null) {
                PSFs[i].array = null;
                PSFs[i].checkExists();
            }
    ...
    public boolean checkPSFArrayNum(int arrayNum) {
        if ((PSFs != null) && (arrayNum >= 0) && (arrayNum < PSFs.length)) {
            return true;
        }
        return false;
    }
    ...
    public boolean checkVoxelBounds(boolean PBC, int xVoxel, int yVoxel, int zVoxel) {
        ...
        if (PBC) {
            ...
            if ((xVoxel >= 0) && (yVoxel >= 0) && (zVoxel >= 0) && (xVoxel < voxelSpacePBC.length) && (yVoxel < voxelSpacePBC[0].length) &&
            ...
            if ((xVoxel >= 0) && (yVoxel >= 0) && (zVoxel >= 0) && (xVoxel < voxelSpace.length) && (yVoxel < voxelSpace[0].length) &&
    ...
    public boolean createVector(boolean close) {
        ...
        if (PSFs != null) {
            for (int i = 0; i < PSFs.length; i++) {
                if (PSFs[i] != null) {
                    addBlankParam();
                    PSFs[i].createVector(true);
                    addVector(PSFs[i].getVector());
                    PSFs[i].addUser(this);
                }
            }
        }
