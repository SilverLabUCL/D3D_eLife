...
final public class Grid
    ...
    public void initVoxelWidth(int w, int h) {
        ...
        if (w < 0) {
            ...
            w = 500; // Math.min(getPreferredSize().width, 500);
        }

        if (h < 0) {
            //h = Math.min(getHeight(), 500);
            h = 500; // Math.min(getPreferredSize().height, 500);
        }
        ...
        //Master.log("pixelsPerVoxel = " + pixelsPerVoxel);
    ...
    private Color displayColorDiffusant(int i, int j, int k, double min, double max) {
        ...
        switch (axesMode) {
            case 0: // xy
                return displayColorDiffusant2(i, j, k, min, max, concentration);
            case 1: // yz
                return displayColorDiffusant2(k, i, j, min, max, concentration);
            case 2: // zx
                return displayColorDiffusant2(j, k, i, min, max, concentration);
        }
    ...
    private Color displayColorDiffusant2(int xVoxel, int yVoxel, int zVoxel, double min, double max, double concentration) {
        ...
        if ((diffusants != null) && (d < diffusants.length)) {

            if (d < 0) {

                for (int ii = 0; ii < diffusants.length; ii++) {

                    if (diffusants[ii] == null) {
                        continue;
                    }

                    c = diffusants[ii].displayColor(xVoxel, yVoxel, zVoxel, min, max);
                ...
                if (concentration == -999) {
                    return diffusants[d].displayColor(xVoxel, yVoxel, zVoxel, min, max);
                } else {
                    return diffusants[d].displayColor(xVoxel, yVoxel, zVoxel, min, max, concentration);
                }
            }
        }
    ...
    private Color displayColorSource2(int xVoxel, int yVoxel, int zVoxel, double min, double max) {
        ...
        if ((sources == null) || (d >= sources.length)) {
            return null;
        }
        ...
        if (d < 0) {
            ...
            for (int ii = 0; ii < sources.length; ii++) {
                ...
                c = sources[ii].displayColor(xVoxel, yVoxel, zVoxel, min, max);
            ...
            return sources[d].displayColor(xVoxel, yVoxel, zVoxel, min, max);
    ...
    private Color displayColorDetector2(int xVoxel, int yVoxel, int zVoxel, double min, double max) {
        ...
        if ((detectors != null) && (d < detectors.length)) {

            if (d < 0) {

                for (int ii = 0; ii < detectors.length; ii++) {

                    if (detectors[ii] == null) {
                        continue;
                    }

                    c = detectors[ii].displayColor(xVoxel, yVoxel, zVoxel, min, max);
                ...
                return detectors[d].displayColor(xVoxel, yVoxel, zVoxel, min, max);
            }
        }
    ...
    public void updateCursorStats() {
        ...
        double iPosition = -1, jPosition= -1, kPosition = -1;
    ...
    private void paintVoxelGrid(Graphics g, boolean isym, boolean jsym) {
        ...
        ColorD3D color = new ColorD3D("PSF", "Heat");

        Color spaceColor = Master.project.geometry.colorSpace.color;
        Color nonSpaceColor = Master.project.geometry.colorNonSpace.color;
        Color tempColor1 = null, tempColor2 = null;
            ...
            for (int j = 0; j < jvox; j++) {
                ...
                if (preview) {
                    ...
                    if (space < 0) {
                        ...
                        g.setColor(displayColorDiffusant(i, j, kVoxel, 0, maxC0));
                    }
                    ...
                    if ((space >= 0) && ((displayMode == 1) || (displayMode == 4))) { // diffusants
                        ...
                        tempColor1 = displayColorDiffusant(i, j, kVoxel, 0, maxC0);
                    ...
                    if ((space >= 0) && (detectors != null) && ((displayMode == 3) || (displayMode == 4))) { // detectors
    ...
    private void paintMonteCarloDiffusants(Graphics g) {
        ...
        for (int i = 0; i < diffusants.length; i++) {
            ...
            if (diffusants[i] instanceof DiffusantVesicles) {
                ...
                dvs = (DiffusantVesicles) diffusants[i];

                if (dvs.vesicle == null) {
                    break;
                }

                for (int j = 0; j < dvs.vesicle.length; j++) {

                    if (dvs.vesicle[j] == null) {
                        continue;
                    }

                    dv = dvs.vesicle[j];

                    if (dvs instanceof DiffusantVesiclesAZ) {
                        color = getMonteCarloColor((DiffusantVesiclesAZ) dvs, (DiffusantVesicleAZ) dv);
                    } else {
                        color = getMonteCarloColor(dvs, dv);
                    }

                    paintMonteCarloDiffusantVesicle(g, dv, color, monteCarlo.PBC);
    ...
    private Color getMonteCarloColor(DiffusantVesicles dvs, DiffusantVesicle dv) {

        Color color = Color.YELLOW;

        DiffusantVesicleAZ dvaz;

        if (dv.insideGeometry) {

            if (Master.project.monteCarlo.frapOn) {
                color = dvs.colorReady.getColor(dv.fluorescence, 0, 1);
            } else {
    ...
    private Color getMonteCarloColor(DiffusantVesiclesAZ dvs, DiffusantVesicleAZ dv) {
        ...
        if (dv.isReady) {
            ...
            if ((montecarlo.azDV != null) && (montecarlo.azDV.connectTo != null)) {
                for (int i = 0; i < montecarlo.azDV.connectTo.length; i++) {
                    if (dv == montecarlo.azDV.connectTo[i]) {
                        color = Color.PINK;
                    }
                }
            }
    ...
    private void paintMonteCarloDiffusantVesicle(Graphics g, DiffusantVesicle dv, Color color, boolean PBC) {

        double xvoxel, yvoxel, zvoxel;
        double ivoxel, jvoxel, kvoxel, halfvoxelwidth;
        double addx = 0, addy = 0, addz = 0, addi = 0, addj = 0;

        int i0, j0, vesiclePixels;

        boolean beyondLimits = false;
    ...
    private void paintMonteCarloMito(Graphics g) {
        ...
        Coordinates[] mito = Master.project.monteCarlo.mito;
        ...
        for (int i = 0; i < mito.length; i++) {
            ...
            paintCoordinates(g, mito[i], mito[i].color.color, fill);
    ...
    private void paintMonteCarloTheActiveZoneEM(Graphics g) {
        ...
        if (false && (mc.activeZoneTethering != null)) {
            ...
            if (mc.displayAZboundariesForQuickDisplay) {
                ...
                if (mc.azXYZ != null) {
                    for (int ipnt = 0; ipnt < mc.azXYZ[mc.EMseriesAZ].length; ipnt+=skip) {
